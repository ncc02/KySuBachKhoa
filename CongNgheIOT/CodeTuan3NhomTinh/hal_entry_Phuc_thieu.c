...
void hal_entry (void)
{
    ...
    while(1)
    {
        ...
        if (hs300x_data.humidity.integer_part > humidity_threshold)
        {
            ...
            R_IOPORT_PinWrite(&g_ioport_ctrl, leds.p_leds[1], BSP_IO_LEVEL_HIGH);
            R_IOPORT_PinWrite(&g_ioport_ctrl, leds.p_leds[2], BSP_IO_LEVEL_HIGH);
        }
        else
        {
            R_IOPORT_PinWrite(&g_ioport_ctrl, leds.p_leds[0], BSP_IO_LEVEL_LOW);  // Tắt đèn đỏ
            R_IOPORT_PinWrite(&g_ioport_ctrl, leds.p_leds[1], BSP_IO_LEVEL_LOW);
            R_IOPORT_PinWrite(&g_ioport_ctrl, leds.p_leds[2], BSP_IO_LEVEL_LOW);
        }
