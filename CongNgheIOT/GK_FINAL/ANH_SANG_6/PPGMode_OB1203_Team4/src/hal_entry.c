...
void hal_entry(void)
{
    ...
    while (1)
    {
        ...
        sprintf (write_buffer, "test %d \n\r", findMax(p_ob1203_data.red_data, p_ob1203_data.green_data, p_ob1203_data.blue_data));
        console_write (write_buffer);
        ...
        uint32_t pin_red_left = leds.p_leds[0];
        uint32_t pin_green_left = leds.p_leds[1];
        uint32_t pin_blue_left = leds.p_leds[2];


        uint32_t pin_red = leds.p_leds[3];
        uint32_t pin_green = leds.p_leds[4];
        uint32_t pin_blue = leds.p_leds[5];
        switch (findMax(p_ob1203_data.red_data, p_ob1203_data.green_data, p_ob1203_data.blue_data))
        {
            case 1:
            {
                ...
                R_BSP_PinWrite((bsp_io_port_t)pin_red_left, BSP_IO_LEVEL_HIGH);

                R_BSP_SoftwareDelay(delay, bsp_delay_units);

                R_BSP_PinWrite((bsp_io_port_t)pin_red_left, BSP_IO_LEVEL_LOW);
            ...
            case 2:
            {
                ...
                R_BSP_PinWrite((bsp_io_port_t)pin_green_left, BSP_IO_LEVEL_HIGH);

                R_BSP_SoftwareDelay(delay, bsp_delay_units);

                R_BSP_PinWrite((bsp_io_port_t)pin_green_left, BSP_IO_LEVEL_LOW);
            ...
            case 3:
            {
                ...
                R_BSP_PinWrite((bsp_io_port_t)pin_blue_left, BSP_IO_LEVEL_HIGH);

                R_BSP_SoftwareDelay(delay, bsp_delay_units);

                R_BSP_PinWrite((bsp_io_port_t)pin_blue_left, BSP_IO_LEVEL_LOW);
                break;
            }
            case 4:
            {
...
int findMax(int a, int b, int c) {
    b = b - 180;
    // a lớn nhất (cách b, c > 200) return 1
    if (a > b + 100 && a > c + 100) {
        return 1;
    }

    // b lớn nhất (cách a, c > 200) return 2
    if (b > a + 100 && b > c + 100) {
        return 2;
    }

    // c lớn nhất (cách a, b > 200) return 3
    if (c > a + 100 && c > b + 180 + 100) {
        return 3;
    }

    // a, b lớn nhất (cách c > 200) return 4
    if (a > c + 100 && b > c + 100) {
        return 4;
    }

    // a, c lớn nhất (cách b > 200) return 5
    if (a > b + 100 && c > b + 100) {
        return 5;
    }

    // b, c lớn nhất (cách a > 200) return 6
    if (b + 180 > a + 100 && c > a + 100) {
        return 6;
    }
