    ...
    while(1){
       ...
       sprintf(write_buffer, "Humidity value: %d.%d \n\r",hs300x_data.humidity.integer_part, hs300x_data.humidity.decimal_part);
        console_write(write_buffer);
         ...
         if (previos  < hs300x_data.humidity.integer_part) val = min(800, val + 200);
         previos = hs300x_data.humidity.integer_part;

         if (hs300x_data.humidity.integer_part  <= 62) val = 0;

         if (hs300x_data.humidity.integer_part > 60 || turnOn == true){
             turnOn = true;


             sprintf(write_buffer, "delay +/- : %d\n\r", val);
              console_write(write_buffer);
